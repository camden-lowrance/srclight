# AGENTS.md — Srclight Project Instructions
...
srclight index --embed embeddinggemma /path/to/repo
...
| `server.py` | FastMCP server, all 29 MCP tools |
...
|-------|----------|------|---------|-------|
| `embeddinggemma` | Ollama | 768 | Good | Smallest modern default (~622MB) |
| `qwen3-embedding:0.6b` | Ollama | varies | Better | Compact higher-quality option |
| `qwen3-embedding` | Ollama | varies | Best local | Heaviest, needs ~6GB VRAM |
| `nomic-embed-text` | Ollama | 768 | Good | Older lightweight fallback |
...
2. Reindex workspace: `srclight workspace index -w <workspace> --embed embeddinggemma`
3. Verify `index_status()` shows new `indexer_version`

### server.json
Version patched automatically during CI from git tag — do NOT bump `server.json` version manually.
